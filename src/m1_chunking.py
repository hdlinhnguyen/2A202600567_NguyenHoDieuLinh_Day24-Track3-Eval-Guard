...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            # Tách chunk mới
            text_chunk = " ".join(current_sentences)
            chunks.append(Chunk(text=text_chunk, metadata={**metadata, "strategy": "semantic", "chunk_index": len(chunks)}))
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        para_len = len(para)
        additional_len = para_len + (2 if current_parent_len > 0 else 0)
        if current_parent_len + additional_len > parent_size and current_parent_paras:
            ...
            current_parent_len += additional_len
    ...
    for parent in parents:
        ...
        for sentence in parent_sentences:
            ...
            additional_len = sent_len + (1 if current_child_len > 0 else 0)
            if current_child_len + additional_len > child_size and current_child_sentences:
                ...
                current_child_len = sent_len
            else:
                current_child_sentences.append(sentence)
                current_child_len += additional_len
